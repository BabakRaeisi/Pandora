...
public class TreasureMapManager : MonoBehaviour
{
    ...
    private void TryShowGatewayPassedPanel()
    {
        ...
        // Unlocking N + 1 means the player completed level N.
        ...
        for (int passedLevel = MinLevel;
             ...
             passedLevel++)
        {
            SWMConfig.LevelConfig passedLevelConfig =
                config.GetLevel(passedLevel);

            if (!config.IsGatewayLevel(passedLevelConfig))
                continue;

            string shownKey = GatewayShownKeyPrefix + passedLevel;

            // Each gateway panel is displayed once only.
            ...
            // Only one gateway panel is shown on each map visit.
            break;
        }
    }

    // Wire the gateway panel close button to this method.
